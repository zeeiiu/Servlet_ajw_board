...
import edu.global.ex.board.command.BReplyCommand;
import edu.global.ex.board.command.BReplyViewCommand;
import edu.global.ex.board.command.BWriteCommand;

/**
 * Servlet implementation class BoardController
 */
@WebServlet("*.do")  //확장자가 .do면 무조건  BoardController로 오게됨.
public class BoardController extends HttpServlet {
	...
	private void actionDo(HttpServletRequest request, HttpServletResponse response)
		...
		String viewPage = null; //viewPage는 BoardController는 요청을 받은 후 나중에 어떠한 view를 보여줄 것인지 역할 
		BCommand command = null; //command 역할을 해줄 객체를 선언 . command는 어떤 로직을 수행할지 결정해주는 역할
		                         // 값이 바뀔 수 있기 때문에 null로 선언.

		String uri = request.getRequestURI();
		String contextPath = request.getContextPath();
		String com = uri.substring(contextPath.length()); //uri 먼저 뽑아낸 녀석에서 substring해서 contextPath의 길이만큼 잘라내고 나면 나머지 뒷 부분 /list.do, content_view.jsp등 만 남음 
		...
		if (com.equals("/list.do")) { // list.do로 치고 들어오게 되면 해당 request 객체 안에 게시판 글들을 가지고 옴.
			...
			System.out.println("/reply_view.do.."); // if문 타는지 확인
			...
			System.out.println("/repl.do.."); // if문 타는지 확인
		...
		RequestDispatcher dispatcher = request.getRequestDispatcher(viewPage); // list.jsp가 viewPage안에 들어감. RequestDispatcher는 포워딩 기법
